/**
 * VoiceTask - Main Logic (API Integrated)
 */
...
async function fetchTasks() {
    try {
        ...
        showToast('Running in Offline Mode (Backend Disconnected)', 'info');
...
async function apiUpdateTask(id, taskData) {
    try {
        ...
        showToast('Updated locally (Backend error)', 'warning');
    }
}
...
async function apiUpdateStatus(id, newStatus) {
    try {
        ...
        showToast('Status changed locally', 'warning');
    }
}
...
async function apiVoiceCommand(command) {
    try {
        ...
        if (!response.ok) throw new Error('Backend could not process command');
        await fetchTasks();
        showToast('Command processed by backend', 'success');
        playBeep(880, 0.2);
    } catch (error) {
    ...
    container.innerHTML = taskArray.map(task => `
        <div class="task-card" data-id="${task.id}" draggable="true">
            ...
            <div class="task-card-footer">
                ...
                <span class="deadline">${task.deadline ? '<i data-lucide="calendar"></i> ' + task.deadline : ''}</span>
            </div>
        </div>
    `).join('');
...
function renderWorkflow() {
    mainView.innerHTML = `
        <div class="workflow-board">
            ...
            <div class="workflow-column" data-status="Completed">
                <div class="column-header">Completed <span class="badge" id="count-completed">0</span></div>
    ...
    const ids = ['col-pending', 'col-progress', 'col-completed'];
    const countIds = ['count-pending', 'count-progress', 'count-completed'];
